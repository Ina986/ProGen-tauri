...
async function launchMojiqFromSave() {
    if (!window.electronAPI || !calibrationSavedFilePath) return;
    if (!window.electronAPI.launchMojiqWithCalibration) {
        showToast('この機能はTauri環境でのみ使用できます', 'error');
        return;
    }

    const btn = document.getElementById('launchMojiqBtn');
    const originalText = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = '起動中...';

    try {
        const result = await window.electronAPI.launchMojiqWithCalibration(calibrationSavedFilePath);
        if (result.success) {
            closeCalibrationSaveSuccessModal();
        } else {
            showToast(result.error || 'MojiQの起動に失敗しました', 'error');
        }
    } catch (error) {
        showToast('MojiQの起動中にエラーが発生しました: ' + error.message, 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = originalText;
    }
